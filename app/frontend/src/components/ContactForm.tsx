    ...
    <div className="hud-corners mt-10 max-w-2xl border border-line bg-panel/70 p-6 sm:p-8">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        <div className="flex flex-col gap-1.5">
          <label className="font-mono text-[10px] text-amber">NOM</label>
          <input
            value={name}
        ...
        <div className="flex flex-col gap-1.5">
          <label className="font-mono text-[10px] text-amber">EMAIL</label>
          <input
            value={email}
        ...
        <div className="flex flex-col gap-1.5 sm:col-span-2">
          <label className="font-mono text-[10px] text-amber">SUJET</label>
          <input
            value={subject}
            onChange={(e) => setSubject(e.target.value)}
            disabled={disabled}
            className="border border-line bg-void px-3 py-2.5 text-sm text-ink placeholder:text-ink/30 focus:border-cyan/60 focus:outline-none"
            placeholder="Mission freelance, opportunité remote…"
          />
        </div>
        <div className="flex flex-col gap-1.5 sm:col-span-2">
        ...
        <div className="sm:col-span-2">
          ...
          >
            {status === "sending" ? "TRANSMISSION EN COURS…" : "ENVOYER LE MESSAGE"}
          </button>
        </div>
      </form>
      ...
      <AnimatePresence mode="wait">
        {status === "sent" && (
          ...
          >
            ✓ MESSAGE REÇU — Bonte te répondra directement par email.
          </motion.div>
        )}
        {status === "mailto" && (
          <motion.div
