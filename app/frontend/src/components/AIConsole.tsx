...
const SUGGESTIONS = ["Quels sont les projets ?", "Quelle est la stack ?", "Comment le contacter ?"];
...
export default function AIConsole() {
  const [messages, setMessages] = useState<Msg[]>([
    { role: "atlas", text: "ATLAS en ligne. Demande-moi les projets, la stack, le parcours ou le contact de Bonte." },
  ]);
  ...
  async function send(text: string) {
    ...
    // petit délai pour laisser la console "réfléchir" — cosmétique, pas un vrai calcul long
  ...
  return (
    <section id="atlas" className="py-16">
      <div className="mx-auto max-w-4xl px-6">
        <Reveal>
          <div className="mb-3 font-mono text-xs text-amber">05 // ASSISTANT EMBARQUÉ</div>
          <h2 className="font-display text-2xl font-bold text-ink md:text-3xl">ATLAS</h2>
          <p className="mt-3 max-w-2xl text-ink/70">
            Un guide par mots-clés qui connaît ce portfolio par cœur — pose-lui une question. (Ce n'est pas un
            grand modèle de langage connecté : les réponses viennent des données de la page.)
          </p>
        </Reveal>
        ...
        <Reveal delay={0.1}>
          <div className="hud-corners mt-10 border border-line bg-panel/70">
            ...
            <div className="flex items-center gap-4 border-b border-line px-6 py-4">
              ...
              <div>
                <div className="font-display text-sm font-semibold text-ink">ATLAS · console d'archive</div>
                <div className="font-mono text-[11px] text-cyan-soft/70">
                  {thinking ? "traitement en cours…" : "en attente d'une requête"}
                </div>
              </div>
            </div>
            ...
            <div ref={logRef} className="flex max-h-80 flex-col gap-4 overflow-y-auto px-6 py-6">
              ...
              {thinking && (
                <div className="self-start font-mono text-xs text-cyan-soft/70">
                  <span className="cursor-blink">●</span> analyse des données du portfolio
                </div>
              )}
            </div>
